...
def antiguedad():
    ...
    for x in range(0,trabajador.get_length(),1):
        if (menor<int(trabajador.get_item(x).get_año_ingreso())):
            menor = int(trabajador.get_item(x).get_año_ingreso())
            print(f"El trabajador {trabajador.get_item(x).get_nombre()} ingresó en el año {menor}, tiene menor antigüedad.")
        elif(mayor>int(trabajador.get_item(x).get_año_ingreso())):
            mayor = int(trabajador.get_item(x).get_año_ingreso())
            print(f"El trabajador {trabajador.get_item(x).get_nombre()} ingresó en el año {mayor}, tiene mayor antigüedad.")
